...
public class AccountJDBCTemplate implements AccountDAO
{
	...
	public Account getAccountByToken(Integer sourceID, String accessToken) {
		String SQL = "select * from nokiaapp.account where sourceID = ? AND accesstoken = ?";
        Account account = jdbcTemplateObject.queryForObject(SQL,new Object[]{sourceID, accessToken}, new AccountMapper());
        return account;
	}
}